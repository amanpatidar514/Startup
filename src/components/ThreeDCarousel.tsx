...
const Card = React.memo<CardProps>(({ src, transform, cardW, cardH }) => (
  <div
    className="absolute"
    style={{
      width: cardW,
  ...
  >
    <div
      className="w-full h-full rounded-2xl overflow-hidden bg-ocean-card border border-cyan-500/20
                 shadow-ocean transition-transform duration-300 hover:scale-105 hover:shadow-2xl"
    >
      <img
        src={src}
        alt="Portfolio item"
...
const ThreeDCarousel = React.memo(
  ({
    ...
    // Shuffle images once per mount to show in random order
    ...
    const cards = useMemo(
      ...
      [shuffledImages, images.length, radius]
    );
